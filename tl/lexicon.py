...
坑 #4:計數之前一定要先過 strip_punct()。曾經誤報「移除 64% 語助詞」,
...
def count_filler_a(canon_text: str) -> int:
    """tier A 出現次數。輸入必須是已經 strip_punct 過的(坑 #4)。"""
    return (sum(canon_text.count(f) for f in FILLER_A_ZH)
            + len(FILLER_A_EN.findall(canon_text)))


def count_filler_b(canon_text: str) -> int:
    return (sum(canon_text.count(f) for f in FILLER_B_ZH)
            + len(FILLER_B_EN.findall(canon_text)))


def count_stutter(canon_text: str) -> int:
    """連續重複的「處」數,不是字數。對對對對 = 1 處。"""
    return (len(STUTTER_ZH_2.findall(canon_text))
            + len(STUTTER_ZH_1.findall(canon_text))
            + len(STUTTER_EN.findall(canon_text)))
